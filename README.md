...
Releases are initiated by running [`dev/release.py`](./dev/release.py). This
will first run some checks (such as making sure that there is an existing
changelog entry). It will then generate a git tag, and push it to the remote.
This act will kick off a release pipeline with Github Actions.
